...
def process_risk_detection(
    ...
    if hardcoded is not None:
        return _build_normalized_flags(hardcoded, apply_confidence_threshold=False), "hardcoded"
    ...
    for one_attempt_index in range(_MAX_LLM_ATTEMPTS):
        raw_response = llm_client.request_completion(system_prompt, user_prompt, json_mode=True)
        if raw_response is None:
            continue

        parsed_response = _parse_llm_response(raw_response)
        if parsed_response is not None:
            return _build_normalized_flags(parsed_response.get("flags", []), apply_confidence_threshold=True), "llm"
